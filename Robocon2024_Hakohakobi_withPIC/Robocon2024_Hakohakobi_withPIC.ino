...
uint8_t PompStatus = 0;
uint8_t ConvStatus = 0;
uint8_t motorStatus = 0;

void SendToPIC(uint8_t motor1Speed,uint8_t motor2Speed,uint8_t motor3Speed,uint8_t motor4Speed,uint8_t motorStatus){
  Serial2.write(0xff);
  delay(2);
  Serial2.write(motor1Speed);
  delay(2);
  Serial2.write(motor2Speed);
  delay(2);
  Serial2.write(motor3Speed);
  delay(2);
  Serial2.write(motor4Speed);
  delay(2);
  Serial2.write(motorStatus);
  delay(2);
}

void PompControl(bool status){
  if(status == true){
    PompStatus = 0b00000001;
  }
  else{
    PompStatus = 0b00000000;
  }
}

void ConvControl(bool status){
  if(status == true){
    ConvStatus = 0b00000010; 
  }
  else{
    ConvStatus = 0b00000000;
  }
}

//ボール
void StoreFunction(void){
      ...
      vTaskDelay(1000);
      PompControl(Stop);
      motorStatus = PompStatus + ConvStatus;
      SendToPIC(0, 0, 0, 0,0);
      vTaskDelay(1000);
}
...
void processGamepad(ControllerPtr ctl) {
  //xが押されたとき
    ...
    // //満点
    // if(ctl->l1()){
    //  LaunchStatus = 0b00000001;
    // }
    // else{
    //   LaunchStatus = 0b00000000;
    // }
...
void setup() {
    ...
    SC.setPeriodHertz(50);//PWMの周波数を50hzに設定(20m周期)
    SC.attach(servoPin, 500, 2400);//サーボのピンとPWMの0度と180度の設定\/?\\/
...
void loop() {
    ...
    //     vTaskDelay(1);
    motorStatus = PompStatus + ConvStatus;
    SendToPIC(motor1_Speed,motor2_Speed,motor3_Speed,motor4_Speed,motorStatus);
